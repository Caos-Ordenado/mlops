...
## Request DTO (shared)
See `shared/shared/shared/interfaces/renderer.py`.
...
class RendererScreenshotRequest(BaseModel):
    ...
    viewport_randomize: bool = False  # small +/- jitter to reduce fingerprint correlation
...
## Anti-bot / Fingerprint behavior

Per request, the renderer applies lightweight fingerprint hardening:
- Realistic **User-Agent rotation** (from a small pool)
- Realistic **locale/timezone**: `es-UY`, `America/Montevideo`
- Realistic **HTTP headers** (`Accept-Language`, `Accept`, `DNT`, `Upgrade-Insecure-Requests`)
- Optional **viewport jitter** when `viewport_randomize=true` (recommended for internal callers)
- **Stealth patches** (via `playwright-stealth`) applied per-page before `goto()`

Stealth is best-effort: if stealth patching fails, the renderer logs and continues.
