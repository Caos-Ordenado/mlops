...
class ContentCacheEntry:
    ...
    def __init__(self, content: str, timestamp: float, ttl_seconds: int = 300):
...
class BatchContentRetriever:
    ...
    async def _check_memory_cache(self, urls: List[str], cached_content: Dict[str, str], missing_urls: List[str]) -> int:
    ...
    async def _check_redis_cache(self, urls: List[str], cached_content: Dict[str, str]) -> int:
        """Check Redis cache for missing URLs."""
        hits = 0
        try:
    ...
    async def _check_database_cache(self, urls: List[str], cached_content: Dict[str, str]) -> int:
        """Check database for cached webpage content."""
        hits = 0
        try:
            ...
            async with db_manager.get_session() as session:
                for url in urls:
                    ...
                    if webpage and webpage.full_text:
                        cached_content[url] = webpage.full_text
                        # Update memory cache
                        self._add_to_memory_cache(url, webpage.full_text)
                        hits += 1
    ...
    async def _bulk_crawl_missing_urls(self, urls: List[str]) -> Dict[str, str]:
        """Crawl missing URLs in a single bulk request."""
        if not urls:
            return {}
                ...
                if response.success and response.results:
                    for result in response.results:
                        if result.url and result.text:
                            crawled_content[result.url] = result.text
                            logger.debug(f"Successfully crawled {result.url} ({len(result.text)} chars)")
                        else:
    ...
    async def _update_all_caches(self, content_batch: Dict[str, str]) -> None:
